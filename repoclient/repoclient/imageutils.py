...
class imageutils(object):
    ...
    def create_image(self,imagepath):
        ...
        if not path.exists(dir):
            makedirs(dir)
        ret1, fs_size = getstatusoutput("df / | awk '{ if (NR == 2) {print $2} }'")
        ret2, fs_bytes_used = getstatusoutput("df / | awk '{ if (NR == 2) {print $3} }'")
        ret3, image_dirsize = getstatusoutput("df "+dir+" | awk '{ if (NR == 2) {print $3} }'")
        
        if(ret1 or ret2 or ret3):
            raise MountError("df ", "error getting filesystem sizes: \n"+ret1+ret2+ret3)
        
        if(image_dirsize < fs_bytes_used):
